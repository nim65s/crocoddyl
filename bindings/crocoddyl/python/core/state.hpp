...
class StateAbstract_py : public StateAbstract<Type, Scalar>, public bp::wrapper<StateAbstract<Type, Scalar>> {
 public:
  StateAbstract_py(int nx, int ndx) : StateAbstract<Type, Scalar>(nx, ndx) {}

  VectorType<Type, Scalar> zero() { return bp::call<VectorType<Type, Scalar>>(this->get_override("zero").ptr()); }

  VectorType<Type, Scalar> rand() { return bp::call<VectorType<Type, Scalar>>(this->get_override("rand").ptr()); }

  VectorType<Type, Scalar> diff(const VectorType<Type, Scalar> &x0, const VectorType<Type, Scalar> &x1) {
    return bp::call<VectorType<Type, Scalar>>(this->get_override("diff").ptr(), x0, x1);
  }

  VectorType<Type, Scalar> integrate(const VectorType<Type, Scalar> &x, const VectorType<Type, Scalar> &dx) {
    return bp::call<VectorType<Type, Scalar>>(this->get_override("integrate").ptr(), x, dx);
  }

  MatrixType<Type, Scalar> Jdiff(const VectorType<Type, Scalar> &x0, const VectorType<Type, Scalar> &x1,
                                 std::string firstsecond = "both") {
    return bp::call<MatrixType<Type, Scalar>>(this->get_override("Jdiff").ptr(), x0, x1, firstsecond);
  }

  MatrixType<Type, Scalar> Jintegrate(const VectorType<Type, Scalar> &x, const VectorType<Type, Scalar> &dx,
                                      std::string firstsecond = "both") {
...
void exposeStateAbstract() {
  bp::class_<StateAbstract_py<Type, Scalar>, boost::noncopyable>("StateAbstract",
...
:param ndx: dimension of state tangent vector)"))
      .def("zero", pure_virtual(&StateAbstract_py<Type, Scalar>::zero), bp::args(" self"),
           R"(Return a zero reference state.

:return zero reference state)")
      .def("rand", pure_virtual(&StateAbstract_py<Type, Scalar>::rand), bp::args(" self"),
           R"(Return a random reference state.

:return random reference state)")
      .def("diff", pure_virtual(&StateAbstract_py<Type, Scalar>::diff), bp::args(" self", " x0", " x1"),
...
:param x1: next state
:return x1 [-] x0 value)")
      .def("integrate", pure_virtual(&StateAbstract_py<Type, Scalar>::integrate), bp::args(" self", " x", " dx"),
...
:return the partial derivative(s) of the diff(x0, x1) function)")
      .def("Jintegrate", pure_virtual(&StateAbstract_py<Type, Scalar>::Jintegrate),
...
:return the partial derivative(s) of the integrate(x, dx) function)")
      .add_property("nx", &StateAbstract_py<Type, Scalar>::nx, "dimension of state configuration vector")
      .add_property("ndx", &StateAbstract_py<Type, Scalar>::ndx, "dimension of state tangent vector")
